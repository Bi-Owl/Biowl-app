...
بعد از اینکه پروژه را با موفقیت مستقر کردید، برای اعمال تغییرات جدیدی که در کد منبع پوش شده است، مراحل زیر را دنبال کنید:

1.  **ورود به پوشه پروژه:**
    ```bash
    cd /var/www/biowl-app
    ```

2.  **دریافت آخرین تغییرات:**
    دستور زیر آخرین تغییرات را از ریپازیتوری گیت‌هاب دریافت می‌کند.
    ```bash
    git pull
    ```

3.  **نصب وابستگی‌ها (در صورت نیاز):**
    اگر پکیج‌های جدیدی به بک‌اند یا فرانت‌اند اضافه شده باشد، باید وابستگی‌ها را دوباره نصب کنید.
    ```bash
    # برای بک‌اند
...
4.  **ری‌استارت سرور بک‌اند:**
    برای اینکه تغییرات کد بک‌اند اعمال شوند، باید سرویس `pm2` را ری‌استارت کنید.
    ```bash
    pm2 restart biowl-backend
    ```

5.  **بیلد مجدد فرانت‌اند:**
    فایل‌های جدید فرانت‌اند باید دوباره ساخته شوند.
    ```bash
    cd frontend
    npm run build
    ```

بعد از انجام این مراحل، وب‌سایت شما با آخرین تغییرات در دسترس خواهد بود. نیازی به ری‌استارت کردن Nginx نیست مگر اینکه فایل کانفیگ آن را تغییر داده باشید.
